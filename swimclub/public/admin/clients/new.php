...
if(is_post_request()) {
    ...
    if(!find_user_by_email($user['email'])){
        ...
        if($result === true) {
            ...
            redirect_to(url_for('/admin/clients/index.php?message=' . h("Client successfully added")));
        } 
        else {
            $errors = $result;
        }
...
<div id="content">
    ...
    <form class="container" method="post" action="new.php">
        ...
        <h1>Add Client</h1><p class="text-danger"><?php echo $message ?? ""; ?></p>
        <div class="form-row">
            ...
            <input type="text" name="last_name" class="form-control" id="last_name" placeholder="Last Name" value="<?php echo h($user['last_name'])?>">
            </div>
        </div>
